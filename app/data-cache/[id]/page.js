...
export default async function Page({ params }) {
    ...
    return (
        <div>
            ...
            <div className="mt-10 flex rounded gap-6 border-zinc-800  w-4xl h-40 items-center justify-center font-bold text-2xl">
                <h1>{product.title}</h1>
            ...
            <div className="flex gap-6 justify-end mt-10 border rounded border-zinc-900 p-10">
                <form action={onRevalidateTagAction}>
                    <button
                        type="submit"
                        className="border border-zinc-800 p-3 rounded cursor-pointer hover:"
                    >
                        Revalidate
                    </button>
                </form>
